...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate an advanced Isaac Franka temporal checkpoint.")
    ...
    model.eval()
    loader = DataLoader(ds, batch_size=int(data_cfg.get("batch_size", 8)), shuffle=False, num_workers=0)
    ...
    report = {k: v / max(n, 1) for k, v in totals.items()}
    report["num_windows"] = n
